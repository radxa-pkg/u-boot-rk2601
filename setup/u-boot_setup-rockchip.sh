...
idblock() {
	if [ -e "$SCRIPT_DIR/idblock_SPI_NAND.img" ]; then
...
rkboot() {
	if [ -e "$SCRIPT_DIR/rkboot_UART0_SD_NAND.bin" ]; then
...
build_spinor() {
	rm -f /tmp/spi.img
	if [[ -f "$SCRIPT_DIR/idblock_SPINOR.img" ]] && [[ -f "$SCRIPT_DIR/u-boot.itb" ]]; then
	...
	elif [[ -f "$SCRIPT_DIR/idblock-spi.img" ]] && [[ -f "$SCRIPT_DIR/uboot.img" ]] && [[ -f "$SCRIPT_DIR/trust.img" ]]; then
...
maskrom_spinor() {
	if [[ -f "$SCRIPT_DIR/rkboot_SPINOR.bin" ]]; then
...
maskrom_spinand() {
	if [[ -f "$SCRIPT_DIR/rkboot_SPI_NAND.bin" ]]; then
